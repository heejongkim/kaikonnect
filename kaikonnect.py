...
cmd_str_tmp="{0}docker run --rm -v {1}:/taggraph_input/ inf/taggraph bash -c \"cd /taggraph_input/ && python /opt/bio/tools/taggraph/TagGraph.1.7.1/scripts/BuildFMIndex.py -f {2} && ls && cd /taggraph_input/ && cd /opt/bio/tools/taggraph/TagGraph.1.7.1/ && python runTG.py /taggraph_input/config/tg_template.params && mv /opt/bio/tools/taggraph/TagGraph.1.7.1/{3}/ /taggraph_input/{3}/  \"".format(sudo_str, tg_input_dir, "/taggraph_input/{0}".format(os.path.basename(options.fasta)),options.tg_ExperimentName)
print "\n\n\nPROGRESS: About to execute command : ",cmd_str_tmp,"\n\n\n"
os.system(cmd_str_tmp)
